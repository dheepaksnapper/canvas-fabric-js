var canvas, brushSize, brushColor, strokeColor, fillColor, f = fabric.Image.filters, cacheStack = [], currentState = 0, isCache;
...
function clipObject (object) {
    if (object.clipTo) {
    object.clipTo = null;
    }
    else {
    var radius = object.width < object.height ? (object.width / 2) : (object.height / 2);
...
function propertiesChangedEvntHandler () {
    canvas.getActiveObjects().forEach(object => {
        let value = this.value, name = this.name
        if (name == 'shadow') {
            enableShadow(object);
            return;
        }
        ...
        if (name == 'strokeWidth') value = parseInt(value, 10);
        else if (name == 'opacity') value = value / 100;
...
function enablePropertiesChanged () {
    $.each($('#properties-div input'), function () {
        $(this).on('change', propertiesChangedEvntHandler); 
    });
...
function updateCache() {
    if (isCache) {
        currentState++;
        cacheStack.push(JSON.stringify(canvas));
...
function undoCanvas () {
    ...
    canvas.clear();
    currentState--;
    console.log(cacheStack[currentState - 1]);
    isCache = false;
    canvas.loadFromJSON(cacheStack[currentState - 1]);
    canvas.renderAll();
    isCache = true;
}
...
function redoCanvas () {
    console.log('redo');
    if(currentState > cacheStack.length) {
        console.log('cannot redo');
        return;
    }
    ...
    currentState++;
    isCache = false;
    canvas.loadFromJSON(cacheStack[currentState]);
    canvas.renderAll();
    isCache = true;
}
...
$(document).ready(function () {
    ...
    $('#btn-redo').on('click', redoCanvas);

    $('#btn-undo').on('click', undoCanvas);
